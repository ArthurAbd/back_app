...
router.post('/getPhoneNumber', v.validate(v.GET_NUMBER_SCHEMA),
...
router.post('/updateInCallRating', v.validate(v.UPD_IN_CALL_SCHEMA),
    passport.authenticate('bearer', { session: false }), 
        async (req, res) => {
            try {
...
router.post('/updateOutCallRating', v.validate(v.UPD_OUT_CALL_SCHEMA),
    passport.authenticate('bearer', { session: false }), 
        async (req, res) => {
            try {
